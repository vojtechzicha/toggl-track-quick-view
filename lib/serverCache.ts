...
//  - Keyed by API path only (the query string is ignored): the time-entries
//    range is "this week up to ~now", which is equivalent for every device
//    within a refresh window, so a value fetched for one device is valid for
//    all. The first caller's range is the one actually fetched.
...
export async function cachedToggl(
  ...
  const key = path; // query intentionally excluded — see file header
  const now = Date.now();
  const cached = store.get(key);
  if (cached && now - cached.at < intervalSec * 1000) return cached;
  ...
  if (!pending) {
    pending = fetchUpstream(path, search, auth)
      .then((entry) => {
        ...
        if (entry.status >= 200 && entry.status < 400) store.set(key, entry);
        return entry;
      })
